...
import { navigationMenu } from "@/assets/menuData";
...
const Sidebar = ({ open, setOpen }: SidebarProps) => {
  ...
  return (
    <motion.aside
      ...
      className={cn(
        "fixed inset-y-0 left-0 z-20 flex flex-col border-r border-slate-700/50 bg-linear-to-b from-slate-900 via-slate-800 to-slate-900 shadow-2xl transition-all duration-300",
        open ? "w-64" : "w-20"
      )}
    >
      ...
      {/* Sidebar menu */}
      <div className="flex flex-col gap-1 flex-1 p-3 bg-linear-to-b from-slate-900/50 to-slate-800/50">
      ...
      {/* Logout button */}
      <div className="p-4 border-t border-slate-600/50 bg-linear-to-r from-slate-800 via-slate-700 to-slate-800">
        <motion.div
          className={cn(
        ...
        >
          <div className="h-10 w-10 rounded-full bg-linear-to-br from-[#29beb3] to-[#a96bde] flex items-center justify-center text-white font-semibold overflow-hidden shadow-lg ring-2 ring-white/20">
            {user?.avatar ? (
              <img
                src={user?.avatar}
                alt="userImage"
...
function NavItem({ to, icon: Icon, label, open, end, pathname }: NavItemProps) {
  return (
    <NavLink
      to={to}
      end={end}
      className={cn(
        ...
        pathname === to
          ? "bg-linear-to-r from-[#29beb3]/20 to-[#a96bde]/20 text-white shadow-lg shadow-[#29beb3]/20 scale-105 ring-1 ring-[#29beb3]/30 border border-white/10 backdrop-blur-sm"
