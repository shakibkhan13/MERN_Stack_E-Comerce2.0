...
const Products = () => {
  ...
  const fetchProducts = async (resetPage = false) => {
    ...
    try {
      ...
      setTotal(response.data.total || productsArray.length);
      setTotalPages(
        response.data.totalPages || Math.ceil((response.data.total || productsArray.length) / perPage)
      );
  ...
  const handleUpdateProduct = async (data: FormData) => {
    ...
    setFormLoading(true);
    try {
      await axiosPrivate.put(`/products/${selectedProduct._id}`, data);
      toast.success("Product updated successfully");
      formEdit.reset(); // reset edit form
      setIsEditModalOpen(false);
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-border/50 bg-muted/30">
                    ...
                    {isAdmin && (
                      <TableHead className="text-right font-semibold min-w-25">
                        Actions
                      </TableHead>
                    )}
                  </TableRow>
